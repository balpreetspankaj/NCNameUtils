module NCNameUtils
  ...
  def isNCNameChar(c)
    return (isAsciiBaseChar(c) || isAsciiDigit(c) || c.chr == '.' ||
      c.chr == '-' || c.chr == '_' || isNonAsciiBaseChar(c) || 
      isNonAsciiDigit(c) || isIdeographic(c) || isCombiningChar(c) ||
      isExtender(c))
  end

  def _charInRange(c, start, stop)
  ...
  def isIdeographic(c)
    return (_charInRange(c, 0x4E00, 0x9FA5) ||
        c == 0x3007 ||
